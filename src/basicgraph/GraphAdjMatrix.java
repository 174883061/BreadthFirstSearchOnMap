...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getNeighbors(int v) {
		...
		for (int i = 0; i < getNumVertices(); i ++) {
			//It will take multiple edges between the same beginning and ending vertices into account.
			for (int j=0; j< adjMatrix[v][i]; j ++) { // iterate through v's row
				neighbors.add(i);
			}
		}
		return neighbors;
...
//				List<Integer> neighbors = this.getNeighbors(i);
//				if(neighbors != null){
...
//		}
//		return twoHop;
		List<Integer> distance1 = this.getNeighbors(v);
		List<Integer> distance2 = new ArrayList<>();
		for(int i : distance1){
			List<Integer> temp = this.getNeighbors(i);
			for(int j : temp){
				distance2.add(j);
			}
		}
